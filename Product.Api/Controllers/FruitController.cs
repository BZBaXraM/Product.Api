...
[ApiController]
public class FruitController(ProductContext context, IMapper mapper) : ControllerBase
{
    ...
    {
        var products = await _service.GetFruitsAsync();
    ...
    {
        var product = mapper.Map<Fruit>(request);
        var newProduct = await _service.CreateFruitAsync(product);
